...
#
# Start script generated by ServerPackCreator 5.1.1.
#
...
echo "Start script generated by ServerPackCreator 5.1.1."
...
setup_playit() {
  ...
  # Instalar Playit si no está instalado
  ...
  # Iniciar Playit en segundo plano con log limitado a 15s
  (
    playit & 
    PLAYIT_PID=$!
    sleep 15
    exec >/dev/null 2>&1  # Redirige toda salida después de 15s
  ) > playit.log 2>&1 &
  
  # Guardar PID para poder terminarlo después
  pgrep -f "playit" > playit.pid
  
  # Esperar a que Playit inicie
  sleep 5
  
  # Mostrar estado de Playit
  if ps -p $(cat playit.pid) > /dev/null; then
    echo "Playit iniciado correctamente"
    head -n 20 playit.log
  else
    echo "Error al iniciar Playit"
    exit 1
  fi
}
