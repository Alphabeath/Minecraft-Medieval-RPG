...
setup_playit() {
  ...
  # Verificar si el proceso sigue ejecutándose usando kill -0
  if kill -0 $PLAYIT_PID 2>/dev/null; then
    echo "Playit iniciado correctamente con PID: $PLAYIT_PID"
    
    # Mostrar las primeras líneas del log si existe
    if [[ -f "playit.log" ]]; then
      echo "Últimas líneas del log de Playit:"
      timeout 2s tail -f playit.log || head -n 10 playit.log
    fi
  else
